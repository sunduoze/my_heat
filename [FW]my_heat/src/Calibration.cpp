...
void ShowCurveCoefficient(void)
{
    ...
    for (long i = 0; i < 4; i++)
    {
        sprintf(buffer, (const char *)"P[%ld]=%.8f\n", i, PTemp[i]);
...
void CalibrationTemperature(void)
{
    ...
    for (uint8_t i = 0; i < FixNum;)
    {
        ...
        if (ADC != -1)
        {
            last_temp = ADC;//LastADC = ADC;

            if (last_temp < SetADC) SetPOWER(255);
            else SetPOWER(0);
        }

        //绘制参考文字

        sprintf(buffer, (const char *)"目标:%ld°C", Calibration_Base[i]);
        DrawHighLightText(128 - Disp.getUTF8Width(buffer) - 2, 21, buffer);

        sprintf(buffer, (const char *)"采样温度 %d", last_temp);
        DrawHighLightText(128 - Disp.getUTF8Width(buffer) - 2, 36, buffer);

        sprintf(buffer, (const char *)"设定ADC：%d", SetADC);
        DrawHighLightText(128 - Disp.getUTF8Width(buffer) - 2, 51, buffer);
    ...
    if (!ExitCalibration_Flag)
    {
        ...
        Pop_Windows((char *)"曲线拟合完成!");
        delay(800);
        ShowCurveCoefficient();
    } else
    {
        Pop_Windows((char *)"取消校准");
        delay(1000);
    }
