...
float MAX6675::getTemperature(uint16_t rawValue)
{
  ...
  if (detectThermocouple(rawValue) != true || getChipID(rawValue) != MAX6675_ID)
  {
    if (detectThermocouple(rawValue) != true)
    {
      Serial.println(F("K-Thermocouple is broken, unplugged or 'T-' terminal is not grounded"));
    }
    if (detectThermocouple(rawValue) != true)
    {
      Serial.println(F("getChipID fail"));
    }
...
// #include <SPI.h>
// // SPIClass SPI_2(MAX6675_MISO, -1, MAX6675_SCK);
...
//     SPI.begin(MAX6675_SCK, MAX6675_MISO, -1, -1);
...
//     digitalWrite(MAX6675_CS, LOW);
//     delay(1);
//     raw_temp = SPI.transfer16(temp);
    
//     delay(1);
//     digitalWrite(MAX6675_CS, HIGH);

//     printf("raw_temp:0x%x\n", raw_temp);
